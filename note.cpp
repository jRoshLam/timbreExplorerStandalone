...
// Set MIDI note
void Note::setMidiNote(int noteNumber)
{
	midiNote_ = noteNumber;
...
float Note::process(Gui& gui, bool noteOn, int noteNumber)
{
	// // Check MIDI messages
	...
	// In standalone, have envelope controlled by noteOn input
	...
	// change frequency
	float noteFrequency = midiToFreqTable_[noteNumber];
	// setMidiIn will only fully run if frequency or qFactor has actually changed.
	// for now, qFactor will be locked at 1.0. index (polyphony) still not active, locked to 0.
	setMidiIn(noteFrequency, 1.0, 0);
