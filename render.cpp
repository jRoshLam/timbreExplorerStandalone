...
// Analog Inputs =======================================================
int gTimbrePotPins[4] = {0, 1, 2, 3};
int gGainPotPin = 4;
float gMax33Vin = 3.3/4.096;
#define MAX_GAIN_INDEX 100
float gGain = 1.0;
// float gGainTable[MAX_GAIN_INDEX] = {0};


// Digital Inputs =======================================================
const int NUM_BUTTONS = 3;
int gButtonPins[NUM_BUTTONS] = {0, 1, 2};
bool gButtonStates[NUM_BUTTONS] = {0};
int gDebounceCounters[NUM_BUTTONS] = {0};
int gDebounceFrames = 0;
float gDebounceTime = 0.01; //10ms
...
bool setup(BelaContext *context, void *userData)
{
	...
	for (unsigned int i = 0; i < NUM_BUTTONS; i++)
		gButtonStates[i] = false;
		
	gDebounceFrames = gDebounceTime * context->audioSampleRate;
	
	// gain lookup table
	// for (int i = 0; i <= MAX_GAIN_INDEX; i++)
	// {
	// 	gGainTable[i] = powf(100, float(i - MAX_GAIN_INDEX)/MAX_GAIN_INDEX);
	// }
...
void render(BelaContext *context, void *userData)
{
	...
	// in this version the timbre parameters are only controlled by the pots
	...
	// update timbre dimensions with potentiometer
	for (int i = 0; i < 4; i++)
	{
		float potValue = analogRead(context, 0, gTimbrePotPins[i]);
		// for some reason, we are exceeding the max theoretical analog reading - compensating by using 246 instead of 255
		int timbreValue = (int)map(potValue, 0, gMax33Vin, 0, 246.3);
		
		// to prevent jitter, only change timbre dimensions if change greater than 1
		if (abs(gTimbreDim[i] - timbreValue) > 1)
		{
			gTimbreDim[i] = timbreValue;
			gTimbreBuffer[2*i] = 1;
			gTimbreBuffer[2*i+1] = gTimbreDim[i];
			switch (i)
			{
				case 0 : gDevNote.setSpectrum(gTimbreDim[i]); break;
				case 1 : gDevNote.setBrightness(gTimbreDim[i]); break;
				case 2 : gDevNote.setArticulation(gTimbreDim[i]); break;
				case 3 : gDevNote.setEnvelope(gTimbreDim[i]); break;
			}
		}
	}
	
	// update volume with potentiometer
	float potValue = analogRead(context, 0, gGainPotPin);
	// for some reason, we are exceeding the max theoretical analog reading - compensating by using 246 instead of 255
	int gainIndex = (int)map(potValue, 0, gMax33Vin, 0, MAX_GAIN_INDEX);
	// gGain = gGainTable[gainIndex];
	gGain = powf(100, float(gainIndex - MAX_GAIN_INDEX)/MAX_GAIN_INDEX);
	...
	for (unsigned int n = 0; n < context->audioFrames; n++)
	{
		...
		{
			// read digital pin
			int newState = digitalRead(context, n, gButtonPins[i]);
			
			// if button is newly pressed or released (with debouncing)
			if (newState != gButtonStates[i] && gDebounceCounters[i] == -1)
			{
				gButtonStates[i] = newState;
				gDebounceCounters[i] = 0;
				
				// button newly pressed
				if (!newState)
				{
					//for note change - increase pitch
					switch (i)
					{
						//pitch down
						case 0 : 
						{
							int newNote = gNoteNumber - gNoteIncr;
							if (newNote >= 0) gNoteNumber = newNote;
							break;
						...
						{
							int newNote = gNoteNumber + gNoteIncr;
							if (newNote <= 255) gNoteNumber = newNote;
							break;
						}
					}
					
					//for new note trigger, nothing to do here, just use gButtonStates[2]
				}
			}
			
			//manage debouncing
			if (gDebounceCounters[i] >= gDebounceFrames)
				gDebounceCounters[i] = -1;
			else if (gDebounceCounters[i] >= 0)
				gDebounceCounters[i]++;
		}
		...
		// call note process
		out = gGain * gDevNote.process(gui, !gButtonStates[2], gNoteNumber);
